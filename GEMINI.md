...
When working with third-party libraries, always consult official documentation to ensure up-to-date information. For efficient documentation lookup, prioritize `context7-mcp`, executing multiple queries in parallel when beneficial. If `context7-mcp` does not yield sufficient results, fall back to `google_web_search` or `web_fetch`, focusing on official sources and concise information.
...
-   **`catalogue-service`**: A backend REST API for product management.
